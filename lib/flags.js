...
/**
 * Text reused across tools that genuinely share a flag's meaning.
 *
 * These are spread into per-tool tables below — never consulted as a global
 * fallback. That fallback existed once and was wrong every time it mattered:
 * FlagTips is mounted in the global MDX wrapper, so it decorates commands.mdx
 * and the write-ups as well as the course this dictionary was scanned from,
 * and an unknown tool would confidently get someone else's answer. `objdump
 * -h` was told it printed help (it prints section headers) and `grep -o`,
 * `ssh -o` and `unzip -o` were all told they wrote output to a file. A tooltip
 * that lies is worse than no tooltip on a site whose promise is that every
 * flag is explained — so an unrecognised (command, flag) pair now says
 * nothing at all.
 */
const HELP = 'Print the help text and exit. The fastest way to check a flag you half-remember.'
const VERBOSE = 'Verbose — print more about what the tool is doing. Add more v’s for more noise.'
const QUIET = 'Quiet — suppress the banner and progress chatter, leaving just the results.'
...
export const FLAGS = {
  ...
  ssh: {
    ...
    '-o': 'Set a config option inline, e.g. -o StrictHostKeyChecking=no. It takes an Option=Value pair, not a filename.'
  },
  chisel: {
  ...
  find: {
    ...
    '-path': 'Match against the whole path rather than just the filename — pair with -not to prune noisy directories.'
  },
  grep: {
    ...
    '-v': 'Invert: show the lines that do *not* match. Nothing like -v on other tools.',
    '-o': 'Print only the matching part of each line, not the whole line. Turns “this 4KB line contains the flag” into just the flag.',
    '-a': 'Treat a binary file as text. Without it grep refuses an executable with “binary file matches” and tells you nothing else.',
    '-b': 'Print the byte offset of each match. That offset is how you find where in the file the hit actually lives.',
    '-c': 'Print only a count of matching lines, not the lines themselves.',
    '-u': 'Report byte offsets without translating CRLF, so -b lines up with the real file.',
    '-d': 'Say what to do when the target is a directory: read, skip or recurse.',
    '-ao': 'The -a and -o pair: read the binary as text and print only the matched substring.',
    '-oE': 'The -o and -E pair: extended regex, and print only what matched.',
    '-aoE': 'Binary-as-text, extended regex, only the match. The one-liner for pulling a pattern out of an executable.',
    '-aob': 'Binary-as-text, only the match, with its byte offset — where in the file the string sits.',
    '-iE': 'Case-insensitive extended regex.',
    '-rn': 'Recurse and show line numbers — the credential-hunting default.',
    '--line-buffered': 'Flush every matching line immediately instead of buffering, so a piped live stream keeps moving.'
  },
  ls: {
  ...
  // ── Reading files & binaries ───────────────────────────────────────────
  // These live outside the course but inside commands.mdx and the write-ups,
  // which FlagTips also decorates. They are here so those pages get a right
  // answer rather than a borrowed one.
  strings: {
    '-a': 'Scan the whole file rather than only the sections a linker marked as data. On a packed or odd binary this is what finds anything.',
    '-n': 'Minimum run length to count as a string. Default 4; raise it to cut noise, lower it to catch short flags.',
    '-t': 'Print the offset of each string, in octal, decimal (d) or hex (x). Pair with -t x to jump straight there in a hex editor.',
    '-e': 'Character width to look for: s for 7-bit ASCII, l for 16-bit little-endian. Windows binaries hide text in UTF-16, which the default misses.'
  },
  objdump: {
    '-h': 'Print the section headers — name, size, file offset and virtual address of each section. Nothing to do with help.',
    '-d': 'Disassemble the executable sections.',
    '-x': 'Everything: headers, symbol table, relocations.',
    '-s': 'Dump the full contents of every section as raw bytes.'
  },
  xxd: {
    '-l': 'How many bytes to dump. Keeps a hexdump of a 40MB binary to the window you actually care about.',
    '-s': 'Seek to this byte offset before dumping. Feed it the offset grep -b just told you.',
    '-p': 'Plain output — bare hex, no offsets or ASCII column. What you want when piping into another tool.',
    '-r': 'Reverse: turn a hexdump back into binary.'
  },
  unzip: {
    '-o': 'Overwrite existing files without prompting. Not an output flag — that trips people who know it from other tools.',
    '-l': 'List the archive contents without extracting anything. Always look before you unpack a hostile sample.',
    '-d': 'Extract into this directory instead of the current one.',
    '-x': 'Exclude these files from the extraction.',
    '-P': 'Supply the archive password on the command line. It lands in your shell history — prefer being prompted.'
  },
  upx: {
    '-d': 'Decompress — unpack an executable UPX previously packed. The first thing to try on a binary whose strings look empty.',
    '-o': 'Write the result to this file instead of modifying the input in place.',
    '-t': 'Test the packed file without writing anything.'
  },
  gitleaks: {
    '-v': VERBOSE,
    '--redact': 'Hide the secret values in the output, showing only where they were found.'
  }
}

// Alias entries that just point at another table.
//
// Keyed WITHOUT the .py: normaliseCommand strips .exe/.py/.sh/.ps1 before the
// lookup, so a `secretsdump.py` key could never be reached. The old script
// names are what older write-ups type; current Kali ships the impacket-* ones.
FLAGS.getnpusers = FLAGS['impacket-getnpusers']
FLAGS.getuserspns = FLAGS['impacket-getuserspns']
FLAGS.secretsdump = FLAGS['impacket-secretsdump']
FLAGS.psexec = FLAGS['impacket-psexec']
FLAGS.wmiexec = FLAGS['impacket-wmiexec']
FLAGS.proxychains4 = FLAGS.proxychains

const FLAG_RE = /^-{1,2}[A-Za-z][A-Za-z0-9?-]*$/

/**
 * A permission string from `ls -l` — -rw-rw-r--, -rwxr-xr-x — passes the flag
 * shape above, because that shape has to admit -p- and -sV alike. Ten
 * characters of exactly the rwx alphabet is not a flag anyone types.
 */
const PERMS_RE = /^-[rwxsStT-]{9}$/

/** True when a token is a flag on its own — never when text is glued to it. */
export function isFlagToken (raw) {
  if (typeof raw !== 'string') return false
  const t = raw.trim()
  if (PERMS_RE.test(t)) return false
...
/**
 * Prefixes that sit in front of the real command and are not it.
 *
 * Without these, `R() { curl … }` in commands.mdx made the command `r()` and
 * every flag inside those blocks lost its tooltip, with the bogus name
 * carried onto the continuation lines too. A numbered step inside a fenced
 * block did the same thing with `1.`.
 */
const PREFIX_RE = [
  /^[A-Za-z_][A-Za-z0-9_]*=/, //     FOO=bar   KRB5CCNAME=x.ccache impacket-psexec …
  /^\d+[.)]$/, //                    1.        a numbered step
  /^[-*]$/, //                       -         a bullet
  /^[A-Za-z_][A-Za-z0-9_]*\(\)$/, //  R()       a shell function being defined
  /^\{$/ //                          {         its opening brace
]
...
function commandOnLine (line) {
  ...
  while (words.length) {
    ...
    if (PREFIX_RE.some((re) => re.test(w))) {
      words = words.slice(1)
      continue
    }
    break
